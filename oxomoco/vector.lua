...
oxomoco.vector = oxomoco.core.class{
    __name = 'oxomoco.vector',
    ...
    validate = function(self, other)
        if not oxomoco.core.isInstance(other, {'table', oxomoco.vector}) then
            error(string.format(
                "vector or table expected, got %s", oxomoco.core.type(other)), 3)
            return false
