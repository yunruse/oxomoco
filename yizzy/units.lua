...
yizzy.units.scinot = function(value, base)
	base = base or 10
	local mg = 0
	repeat
		if value < 1 then
...
yizzy.units.prefix = function(number, digits, longForm, system)
	digits = digits or 3
	system = system or yizzy.units.SI
	longForm = longForm or false
	
	local value, mg = yizzy.units.scinot(number, system.base)
...
yizzy.units.scientific = function(number, digits)
	local value, mg = yizzy.units.scinot(number, system.base)
	value = string.format('%.' .. tostring(digits) .. 'f', value)
	return value ..'×10^' .. mg
end
